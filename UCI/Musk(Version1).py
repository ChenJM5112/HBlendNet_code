...
# 加载数据文件
data1 = pd.read_csv('./data/clean1.data', header=None, sep=',')

# 加载数据的描述信息
with open('./data/clean1.names', 'r') as f1:
    description1 = f1.read()

# 分离数据和标签（假设第一列包含标签）
y = data1.iloc[:, -1]  # 数据集1的标签
X = data1.iloc[:, 2:-1]  # 数据集1的特征
...
model.fit([X_train, X_train], y_train, batch_size=64, epochs=1500, validation_data=([X_test, X_test], y_test), callbacks=[early_stopping])
